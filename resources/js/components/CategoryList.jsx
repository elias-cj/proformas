    ...
    <div className="p-8 w-full max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 relative">
      ...
      <div className="flex justify-between items-center mb-10 bg-white/50 backdrop-blur-md p-6 rounded-[2rem] shadow-xl border border-white/60">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Gestión de familias para productos y servicios</p>
        </div>
        <button 
              ...
              {categories.map((c) => (
                <tr key={c.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="py-4 px-6 flex items-center gap-3">
                    ...
                    <p className={`font-black text-${c.color}-600`}>{c.name}</p>
                  </td>
                  <td className="py-4 px-6 text-sm text-slate-500 font-medium">{c.description}</td>
                  <td className="py-4 px-6 text-center">
                    <span className={`px-3 py-1 text-xs font-bold rounded-full ${
                      c.status === 'Activo' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500'
                    }`}>
                      {c.status}
                    </span>
                  </td>
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-200">
            <div className="p-6 md:p-8">
              ...
              <form onSubmit={handleSave} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Nombre de Etiqueta</label>
                ...
                <div>
                  ...
                  <textarea name="description" rows="2" value={formData.description} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-semibold focus:ring-2 focus:ring-indigo-500 focus:outline-none resize-none"></textarea>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Color de Distintivo</label>
                    <select name="color" required value={formData.color} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-semibold focus:ring-2 focus:ring-indigo-500 focus:outline-none cursor-pointer">
                      {colorOptions.map(c => (
                ...
                <div className="mt-8 pt-6 border-t border-slate-100 flex justify-end gap-3">
                  ...
                  <button type="submit" className={`px-6 py-3 font-bold text-white bg-${formData.color}-500 hover:bg-${formData.color}-600 shadow-lg shadow-${formData.color}-500/30 rounded-xl transition-all active:scale-95`}>
                    Guardar Etiqueta
                  </button>
                </div>
              </form>
