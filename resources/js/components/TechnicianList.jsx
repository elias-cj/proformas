...
export default function TechnicianList() {
  ...
  return (
    <div className="p-8 w-full max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 relative">
      ...
      {/* Header */}
      <div className="flex justify-between items-center mb-10 bg-white/50 dark:bg-neutral-900/50 backdrop-blur-md p-6 rounded-[2rem] shadow-xl border border-white/60 dark:border-neutral-800 transition-all duration-300">
        <div>
          <h2 className="text-3xl font-black bg-gradient-to-r from-[var(--accent)] to-purple-600 bg-clip-text text-transparent">
            Técnicos
          </h2>
          <p className="text-slate-500 dark:text-neutral-400 font-medium mt-1">Directorio y disponibilidad del equipo operativo</p>
        </div>
        <button 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-neutral-800">
              {technicians.map((t) => (
                <tr key={t.id} className="hover:bg-slate-50/50 dark:hover:bg-neutral-800/30 transition-colors group">
                  <td className="py-4 px-6 flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[var(--accent-soft)] to-purple-100 dark:from-[var(--accent-soft)] dark:to-purple-900/40 flex items-center justify-center text-[var(--accent)] dark:text-[var(--accent)] font-bold border border-[var(--accent-soft)] dark:border-[var(--accent-soft)] shadow-sm shrink-0">
                      {t.name.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase()}
                    </div>
                    <div>
                      <p className="font-bold text-slate-800 dark:text-white">{t.name}</p>
                      <p className="text-xs text-slate-400 dark:text-neutral-500 font-medium">{t.doc}</p>
                    </div>
                  </td>
                  <td className="py-4 px-6">
                    <p className="font-semibold text-slate-700 dark:text-neutral-300">{t.phone}</p>
                  </td>
                  <td className="py-4 px-6">
                    <span className="text-sm font-medium text-slate-600 dark:text-neutral-400">{t.specialty}</span>
                  </td>
                  <td className="py-4 px-6 text-center">
                    <div className="flex justify-center items-center gap-2">
                       <span className={`relative flex h-3 w-3`}>
                          ...
                          <span className={`relative inline-flex rounded-full h-3 w-3 ${t.status === 'Disponible' ? 'bg-emerald-500' : t.status === 'En Ruta' ? 'bg-amber-500' : 'bg-slate-300 dark:bg-neutral-700'}`}></span>
                       </span>
                       <span className="text-xs font-bold text-slate-600 dark:text-neutral-400">{t.status}</span>
                    </div>
                  </td>
                  <td className="py-4 px-6 text-center">
                    <span className={`font-black ${t.ordersAssigned > 0 ? 'text-[var(--accent)]' : 'text-slate-400 dark:text-neutral-600'}`}>
                        {t.ordersAssigned}
                    </span>
                  </td>
                  <td className="py-4 px-6">
          ...
          <div className="bg-white dark:bg-[#1a1a1a] rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden border border-slate-100 dark:border-neutral-800 animate-in zoom-in-95 duration-200">
            <div className="p-6 md:p-8">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={handleCloseModal} className="text-slate-400 hover:text-slate-600 dark:hover:text-neutral-300 transition-colors">
              ...
              <form onSubmit={handleSave} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-neutral-500 uppercase tracking-widest mb-1">Nombre Completo</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-neutral-500 uppercase tracking-widest mb-1">Documento (CI)</label>
                  ...
                  <div>
                     <label className="block text-xs font-bold text-slate-500 dark:text-neutral-500 uppercase tracking-widest mb-1">Teléfono Móvil</label>
                     <input type="text" name="phone" required value={formData.phone} onChange={handleChange} className="w-full bg-slate-50 dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 rounded-xl px-4 py-3 text-sm font-semibold text-slate-800 dark:text-white focus:ring-2 focus:ring-[var(--accent)] focus:outline-none transition-all" />
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-500 dark:text-neutral-500 uppercase tracking-widest mb-1">Especialidad Principal</label>
                  <input type="text" name="specialty" placeholder="Ej: Redes, CCTV, Eléctrico" required value={formData.specialty} onChange={handleChange} className="w-full bg-slate-50 dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 rounded-xl px-4 py-3 text-sm font-semibold text-slate-800 dark:text-white focus:ring-2 focus:ring-[var(--accent)] focus:outline-none transition-all" />
                </div>

                <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-neutral-500 uppercase tracking-widest mb-1">Estado Operativo</label>
